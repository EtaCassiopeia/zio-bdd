...
//  Then("an email should be sent to {email:String}") { (input: Any) =>
//    ZIO.fromEither(input match {
//      case e: String if e.contains("@") => Right(e) // Enforce email-like format
//      case _ =>
//        Left(new Exception("Invalid input for Then step: expected a valid email address"))
//    }).flatMap { expectedEmail =>
//      for {
//        _          <- ZIO.serviceWithZIO[LogCollector](_.log(s"Checking emails for: $expectedEmail"))
//        emailSvc   <- ZIO.service[EmailService]
...
////    for {
////      _          <- ZIO.serviceWithZIO[LogCollector](_.log(s"Checking emails for: $email"))
////      emailSvc   <- ZIO.service[EmailService]
////      sentEmails <- emailSvc.getSentEmails
////      _          <- Assertions.assertTrue(sentEmails.contains(email), s"Email $email not found in $sentEmails")
////    } yield ()
////  }
////}
////
...
//////      case (_, e: String) => Right(e)
//////      case _ => Left(new Exception("Invalid input for Then step"))
//////    }).flatMap { expectedEmail =>
//////      for {
//////        _ <- ZIO.serviceWithZIO[LogCollector](_.log(s"Checking emails for: $expectedEmail"))
//////        emailSvc <- ZIO.service[EmailService]
