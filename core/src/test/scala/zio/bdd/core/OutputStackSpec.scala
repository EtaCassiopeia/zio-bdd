  ...
  override def spec: Spec[Any, Nothing] =
    suite("OutputStack")(
      ...
      test("push adds a record to the top of the stack") {
        val record = StepRecord(StepType.GivenStep, "Given I have data", "data", testScenarioId1)
        for {
          stackRef <- OutputStack.make
      ...
      test("peek returns the most recent record for the scenario without removing it") {
        val record1 = StepRecord(StepType.GivenStep, "Given I start", "first", testScenarioId1)
        val record2 = StepRecord(StepType.WhenStep, "When I act", "second", testScenarioId1)
        val record3 = StepRecord(StepType.ThenStep, "Then I check", "third", testScenarioId2)
        for {
          stackRef <- OutputStack.make
      ...
      test("pop returns None when no records exist for the scenario") {
        val record = StepRecord(StepType.GivenStep, "Given I start", "first", testScenarioId2)
        for {
          stackRef <- OutputStack.make
          _        <- OutputStack.push(stackRef, record)
          popped   <- OutputStack.pop(stackRef, testScenarioId1)
      ...
      test("clear empties the entire stack") {
        val record1 = StepRecord(StepType.GivenStep, "Given I have data", "data", testScenarioId1)
        val record2 = StepRecord(StepType.WhenStep, "When I act", "second", testScenarioId2)
        for {
          stackRef <- OutputStack.make
          _        <- OutputStack.push(stackRef, record1)
          _        <- OutputStack.push(stackRef, record2)
...
//      test("getPriorOutput handles type mismatches") {
//        val record = StepRecord(StepType.GivenStep, "Given I start", "first", testScenarioId1)
//        for {
//          stackRef <- OutputStack.make
//          _        <- OutputStack.push(stackRef, record)
//          result   <- OutputStack.getPriorOutput[Int](stackRef, testScenarioId1, List("extra")).exit
//        } yield assert(result)(fails(hasMessage(containsString("Type mismatch: expected Int"))))
//      },
