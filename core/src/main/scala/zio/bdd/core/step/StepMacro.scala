...
object StepMacro {
  ...
  inline def stepDef[R, S, T](
    inline builder: StepPatternBuilder,
    inline f: T => RIO[R with State[S], Unit],
    self: ZIOSteps[R, S]
  ): Unit =
    ${ stepDefImpl[R, S, T]('builder, 'f, 'self) }
  ...
  private def stepDefImpl[R: Type, S: Type, T: Type](
    ...
    Type.of[T] match {
      ...
      case '[(a, b)] =>
        ...
        val patternExpr: Expr[StepPattern[(a, b)]] = '{
          StepPattern2[a, b]($regexExpr, $segmentsExpr, $e1Expr, $e2Expr)
        }
        '{
          $arityCheck
          $selfExpr.register(
      ...
      case '[(a, b, c)] =>
        ...
        val patternExpr: Expr[StepPattern[(a, b, c)]] = '{
          StepPattern3[a, b, c]($regexExpr, $segmentsExpr, $e1Expr, $e2Expr, $e3Expr)
        }
        '{
          $arityCheck
          $selfExpr.register(
