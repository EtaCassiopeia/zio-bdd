...
import zio.bdd.core.report.{ErrorExplainer, Reporter}
  ...
  private def executeMatchedStep[I <: Matchable, O](
    ...
    for {
      ...
      input <- determineInput(params, currentStepType, iTag)
                 ...
                 .catchAll { e =>
                   ZIO.succeed(
                     Left(TestError.TypeMismatch(iTag.tag.toString, e.getMessage, params, Some(e), Some(trace)))
                   )
                 }
      result <- input match {
                  case Left(error) =>
...
//        }
//        end         <- Clock.instant
//        finalResult  = result.copy(duration = Duration.fromInterval(start, end), startTime = start, file = gherkinStep.file, line = gherkinStep.line)
//        _           <- reporter.endStep(gherkinStep.pattern, finalResult)
//        _           <- OutputStack.push(stackRef, StepRecord(gherkinStep.stepType, gherkinStep.pattern, finalResult.output))
//        updatedStack <- stackRef.get
//        _           <- logCollector.log(scenarioId, gherkinStep.id, s"After ${gherkinStep.pattern}, Stack: $updatedStack, Duration: ${finalResult.duration}", InternalLogLevel.Debug)
//      } yield finalResult
//    }
//
//  private def findMatchingStepDef(
...
//        .map(Right(_))
//        .catchAll { e =>
//          ZIO.succeed(Left(TestError.TypeMismatch(iTag.tag.toString, e.getMessage, params, Some(e), Some(trace))))
//        }
//      result <- input match {
//        case Left(error) =>
//          ZIO.succeed(StepResult(line, succeeded = false, error = Some(error), output = (), logs = Nil, duration = Duration.Zero, startTime = start))
//        case Right(value) =>
//          executeStepFunction(stepDef.fn, line, value, start, gherkinStep.id)
//      }
//    } yield result
...
//      for {
//        logs <- logCollector.getLogs(scenarioId, stepId)
//        _    <- logCollector.log(scenarioId, stepId, s"Step execution failed", InternalLogLevel.Error)
//      } yield StepResult(
//        line,
//        succeeded = false,
//        error = Some(TestError.GenericError("Step execution failed", None, Some(trace))),
//        output = (),
//        logs = logs.toStepResultLogs,
//        duration = Duration.Zero,
//        startTime = start
...
//    )
//
//  private def handleStepFailure(gherkinStep: GherkinStep, cause: Cause[Throwable], start: Instant)(implicit trace: Trace): ZIO[R, Nothing, Unit] =
//    for {
//      logs <- logCollector.getLogs(scenarioId, gherkinStep.id)
//      _    <- logCollector.log(scenarioId, gherkinStep.id, s"Step failed: ${cause.prettyPrint}", InternalLogLevel.Error)
//    } yield ()
