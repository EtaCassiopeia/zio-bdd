...
case class LogEntry(
  ...
  stepId: String // Added to track step-specific logs
)
...
object LogCollector {
                  ...
                  _.map(
                    _.add(
                      LogEntry(
                        ...
                        level,
                        stepId // Store stepId in LogEntry
                      )
                    )
                  ).orElse(
                    Some(
                  ...
                  _.map(
                    _.add(
                      LogEntry(
                        ...
                        level,
                        "feature" // Feature-level logs don’t need stepId
                      )
                    )
                  ).orElse(
                    Some(
  ...
  private val loggingLayer: ZLayer[LogCollector, Nothing, Unit] = ZLayer.scoped {
    for {
      ...
      _ <- FiberRef.currentLoggers.locallyScoped(
