...
case class PrettyReporter() extends Reporter {
  ...
  private def printFeature(
    ...
    val featureColor = if (feature.isPassed) LightGreen else LightRed
    val fileInfo     = feature.feature.file.map(f => s" (${shortenFilePath(f)})").getOrElse("")
    val prefix       = if (isLast) LastBranch else Branch
    ZIO.succeed(println(s"${featureColor}$prefix◉ Feature: ${feature.feature.name}$fileInfo - ${
        if (feature.isPassed) "PASSED" else "FAILED"
      }${Reset}")) *>
      ZIO.foreach(feature.scenarioResults.zipWithIndex) { case (scenario, index) =>
        val isLastScenario = index == feature.scenarioResults.size - 1
        printScenario(scenario, logCollector, isLastScenario, if (isLast) Indent else Vertical)
      } *>
  ...
  private def printScenario(
    ...
    val scenarioColor = if (scenarioResult.isPassed) LightYellow else LightRed
    val lineInfo      = scenarioResult.scenario.line.map(l => s":$l").getOrElse("")
    val prefix        = if (isLast) LastBranch else Branch
    val scenarioIdStr = scenarioResult.scenario.id.toString
    ZIO.succeed(println(s"$indent${scenarioColor}$prefix◑ Scenario: ${scenarioResult.scenario.name}$lineInfo - ${
        if (scenarioResult.isPassed) "PASSED" else "FAILED"
      }${Reset}")) *>
  ...
  private def printFeatureSummary(feature: FeatureResult, indent: String): ZIO[Any, Nothing, Unit] = {
    val passed = feature.scenarioResults.count(_.isPassed)
    val failed = feature.scenarioResults.count(!_.isPassed)
    ZIO.succeed(println(s"$indent${LightBlue}Summary: Passed: $passed, Failed: $failed${Reset}"))
  }
  ...
  override def report(results: List[FeatureResult]): ZIO[LogCollector, Throwable, Unit] =
    for {
      ...
      _ <- ZIO.foreach(results.zipWithIndex) { case (feature, index) =>
             val isLastFeature = index == results.size - 1
             printFeature(feature, logCollector, isLastFeature)
           }
      totalPassed = results.flatMap(_.scenarioResults).count(_.isPassed)
      totalFailed = results.flatMap(_.scenarioResults).count(!_.isPassed)
      _          <- ZIO.succeed(println(s"${LightBlue}Final Summary: Passed: $totalPassed, Failed: $totalFailed${Reset}"))
    } yield ()
}
